<template>
  <div class="container">
    <Breadcrumb :items="['menu.list', 'menu.list.Factors']" />
    <a-card class="general-card" :title="$t('menu.list.Factors')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
...
<script lang="ts" setup>
  ...
  const { t } = useI18n();
  const renderData = ref<OtherFactorsRecord[]>([]);
  ...
  // 缓存键
  const CACHE_KEY = 'carbon_factors';
  // 提交查询请求
  const fetchData = async (params: CarbonMarketParams) => {
    ...
    try {
      ...
      // 无缓存发送请求
      const { data } = await queryOtherFactorsList(params);
  ...
  const handleUpload = async (option) => {
    ...
    try {
      ...
      // 调用封装的 API 函数进行文件上传
      const res = await uploadOtherFactors(fileItem);
      if (res.code === 200) {
        if (res.data) {
          Notification.success({
            id: 'upload',
